...
The Nexys Video board comes with 512 MB of DDR3 memory.

The Open-Source DDR3 Controller 'UberDDR3' is used for interfacing.
It is connected to a 256-bit wide 512-entry (16 KB) direct-mapped last-level cache, which uses a 256-bit-wide bus protocol loosely based on TileLink.
...
When the DDR3 Memory Interface is included in RTL simulations, the DDR3 model will throw several errors related to timing.
These can be ignored as the startup/calibration sequence of the DDR3 controller is intentionally shortened for faster simulation (MICRON_SIM=1 in the UberDDR3 instantiation).
